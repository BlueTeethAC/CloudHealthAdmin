    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['activity:activityClassify:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['activity:activityClassify:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
...
<script>
import { listActivityClassify, getActivityClassify, delActivityClassify, addActivityClassify, updateActivityClassify } from "@/api/activity/activityClassify";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listActivityClassify(this.queryParams).then(response => {
        this.activityClassifyList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getActivityClassify(classifyId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.classifyId != null) {
            updateActivityClassify(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addActivityClassify(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除活动分类编号为"' + classifyIds + '"的数据项？').then(function() {
        return delActivityClassify(classifyIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('activity/activityClassify/export', {
        ...this.queryParams
      }, `activityClassify_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>

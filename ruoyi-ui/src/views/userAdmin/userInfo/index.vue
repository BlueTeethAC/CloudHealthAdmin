<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="微信识别码" prop="openId">
        <el-input
      ...
      <el-form-item label="用户角色" prop="userRole">
        <el-select v-model="queryParams.userRole" placeholder="请选择用户角色" clearable>
          <el-option
            v-for="dict in dict.type.ch_user_role"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
      ...
      <el-form-item label="账号状态" prop="userStatus">
        <el-select v-model="queryParams.userStatus" placeholder="请选择账号状态" clearable>
          <el-option
    ...
    <el-table v-loading="loading" :data="userInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="账号状态" align="center" prop="userStatus">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.ch_user_status" :value="scope.row.userStatus"/>
        </template>
      </el-table-column>
...
<script>
import { listUserInfo, getUserInfo, delUserInfo, addUserInfo, updateUserInfo } from "@/api/userAdmin/userInfo";
...
export default {
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('userAdmin/userInfo/export', {
        ...this.queryParams
